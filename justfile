...
[windows]
publish-release:
    $version = (Select-String -Path 'Cargo.toml' -Pattern '^version = "(.+)"' | Select-Object -First 1).Matches.Groups[1].Value; git cliff --latest | gh release create "v$version" --title "divvy-v$version" --notes-file -
...
publish-release:
    ...
    set -euo pipefail
    VERSION=$(grep '^version = ' Cargo.toml | head -1 | sed 's/version = "\(.*\)"/\1/')
    git cliff --latest | gh release create "v$VERSION" --title "divvy-v$VERSION" --notes-file -
